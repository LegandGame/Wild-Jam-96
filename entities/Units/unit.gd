...
var target : Node3D	# enemy unit we are currently moving towards
...
# --- misc and utils
func attack() -> void:
	hitbox.call_deferred("monitorable", true)
	hitbox.call_deferred("monitoring", true)
	# stick an await here for the attack animation
	hitbox.call_deferred("monitorable", true)
	hitbox.call_deferred("monitoring", true)
